...
(* modified by Norayr Chilingaryan to make independent of OBERON variable *)
  IMPORT SYSTEM, Unix, Kernel, Args, Console, IntStr;
...
*)
  ...
  BEGIN
    ...
    i := 0;
    REPEAT 
       IF s[i] >=20X THEN Write (R, s[i]) END; (* nc *)
       INC(i);
    UNTIL (s[i] < 20X);
  END WriteLInt;
